...
export async function syncLeaderboardEntry(studentEmail, patch) {
  try {
    ...
    if (entries && entries.length > 0) {
      ...
      if (entries.length > 1) {
        console.warn(`⚠️ Found ${entries.length} LeaderboardEntry records for ${studentEmail}, cleaning up duplicates...`);
        ...
        for (let i = 1; i < sortedEntries.length; i++) {
          await base44.entities.LeaderboardEntry.delete(sortedEntries[i].id);
          console.log(`🗑️ Deleted duplicate LeaderboardEntry ${sortedEntries[i].id}`);
        }
      }
