// Backend function - NO imports from @/api/base44Client
// Server-side entity access via context parameter
...
export default async function recalculateStudentEconomySnapshot({ studentEmail, reason = 'manual', previewOnly = true } = {}, context) {
  try {
    ...
    // Access entities via context (server-side)
    ...
    const [
      ...
      entities.User?.filter({ email: studentEmail }) || [],
      entities.WordProgress?.filter({ student_email: studentEmail }) || [],
      entities.MathProgress?.filter({ student_email: studentEmail }) || [],
      entities.LessonParticipation?.filter({ student_email: studentEmail }) || [],
      entities.QuizProgress?.filter({ student_email: studentEmail }) || [],
      entities.Investment?.filter({ student_email: studentEmail }) || []
    ]);
    ...
    // PREVIEW ONLY: return computed values without DB write
    if (previewOnly === true) {
      console.log("👁️ Preview mode - no DB write", { studentEmail });
      return { ok: true, ...snapshotData };
    }

    // CREATE/UPDATE StudentEconomySnapshot in database
    const existing = await entities.StudentEconomySnapshot?.filter({ student_email: studentEmail }) || [];

    let saved;
    if (existing && existing.length > 0) {
      saved = await entities.StudentEconomySnapshot.update(existing[0].id, snapshotData);
      console.log("✅ Updated StudentEconomySnapshot", { studentEmail, id: existing[0].id });
    } else {
      saved = await entities.StudentEconomySnapshot.create(snapshotData);
